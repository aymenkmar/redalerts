<div>
    <h1 class="text-2xl font-bold mb-6">Endpoints</h1>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $endpoint['metadata']['namespace'] ?? 'default' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if(isset($endpoint['subsets']))
                                @php
                                    $endpointAddresses = [];
                                    foreach ($endpoint['subsets'] as $subset) {
                                        if (isset($subset['addresses'])) {
                                            foreach ($subset['addresses'] as $address) {
                                                $endpointAddresses[] = $address['ip'] ?? '';
                                            }
                                        }
                                    }
                                @endphp
                                {{ implode(', ', $endpointAddresses) }}
                            @else
                                —
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $this->formatAge($endpoint['metadata']['creationTimestamp']) }}
                        </td>
                    </tr>
                    @empty
                    <tr>
