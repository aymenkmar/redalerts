<div x-data="ingressClassesList()" x-init="init()">
    ...
    <!-- Search Input (No namespace filter since Ingress Classes are cluster-scoped) -->
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center mb-6">
        <div class="relative flex-1 max-w-md">
            <input
                type="text"
                x-model="searchTerm"
                @input="filterIngressClasses()"
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading && filteredIngressClasses.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredIngressClasses.length)"></span>
                of
                <span class="font-medium" x-text="filteredIngressClasses.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredIngressClasses.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredIngressClasses.length / perPage))" :disabled="currentPage >= Math.ceil(filteredIngressClasses.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function ingressClassesList() {
            return {
                allIngressClasses: @json($ingressClasses),
                filteredIngressClasses: [],
                paginatedIngressClasses: [],
                searchTerm: '',
                clientLoading: false,
                currentPage: 1,
                perPage: 25,

                init() { this.filterIngressClasses(); },

                filterIngressClasses() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allIngressClasses];
                ...
                updatePagination() {
                    ...
                    this.paginatedIngressClasses = this.filteredIngressClasses.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredIngressClasses.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredIngressClasses.length / this.perPage);
                    const maxVisible = 5;
