<div>
    <h1 class="text-2xl font-bold mb-6">Network Policies</h1>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($networkPolicies as $policy)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if(isset($policy['spec']['podSelector']['matchLabels']))
                                @php
                                    $selectors = [];
                                    foreach ($policy['spec']['podSelector']['matchLabels'] as $key => $value) {
                                        $selectors[] = "$key=$value";
                                    }
                                @endphp
                                {{ implode(', ', $selectors) }}
                            @elseif(isset($policy['spec']['podSelector']) && empty($policy['spec']['podSelector']))
                                All Pods
                            @else
                                —
                            @endif
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $this->formatAge($policy['metadata']['creationTimestamp']) }}
                        </td>
                    </tr>
                    @empty
                    <tr>
