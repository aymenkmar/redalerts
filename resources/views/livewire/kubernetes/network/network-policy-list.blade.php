<div x-data="networkPoliciesList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterNetworkPolicies();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterNetworkPolicies();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading && filteredNetworkPolicies.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredNetworkPolicies.length)"></span>
                of
                <span class="font-medium" x-text="filteredNetworkPolicies.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredNetworkPolicies.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredNetworkPolicies.length / perPage))" :disabled="currentPage >= Math.ceil(filteredNetworkPolicies.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function networkPoliciesList() {
            return {
                allNetworkPolicies: @json($networkPolicies),
                namespaces: @json($namespaces),
                filteredNetworkPolicies: [],
                paginatedNetworkPolicies: [],
                searchTerm: '',
                ...
                filterNetworkPolicies() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allNetworkPolicies];
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(policy => this.selectedNamespaces.includes(policy.metadata.namespace || 'default'));
                        }
                ...
                updatePagination() {
                    ...
                    this.paginatedNetworkPolicies = this.filteredNetworkPolicies.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredNetworkPolicies.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredNetworkPolicies.length / this.perPage);
                    const maxVisible = 5;
