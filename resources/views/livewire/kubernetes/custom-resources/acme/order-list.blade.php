<div x-data="ordersList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">ACME Orders</h1>
        <button
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center mb-6">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterOrders();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterOrders();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
        ...
        <div class="relative flex-1 max-w-md">
            <input
                ...
                x-model="searchTerm"
                @input="filterOrders()"
                placeholder="Search ACME orders..."
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-8 text-center">
            <p class="text-gray-600">Please select a cluster from the dropdown to view ACME orders.</p>
        </div>
        @else
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="order in paginatedOrders" :key="order.metadata.name + order.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="order.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getNamespace(order)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getState(order)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(order.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>
                    ...
                    <tr x-show="filteredOrders.length === 0">
                        <td colspan="4" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No ACME orders found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No ACME orders found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredOrders.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredOrders.length)"></span>
                of
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredOrders.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredOrders.length / perPage))" :disabled="currentPage >= Math.ceil(filteredOrders.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function ordersList() {
            return {
                ...
                filterOrders() {
                    ...
                    setTimeout(() => {
                        ...
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(order => this.selectedNamespaces.includes(this.getNamespace(order)));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(order => {
                                const name = (order.metadata.name || '').toLowerCase();
                                const namespace = this.getNamespace(order).toLowerCase();
                                const state = this.getState(order).toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower) || state.includes(searchLower);
                            });
                        }
                ...
                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredOrders.length / this.perPage);
                ...
                getNamespace(order) {
                    return (order.metadata && order.metadata.namespace) || 'default';
                },

                getState(order) {
                    return (order.status && order.status.state) || 'N/A';
                },
