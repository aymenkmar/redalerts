<div x-data="challengesList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">ACME Challenges</h1>
        <button
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center mb-6">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterChallenges();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterChallenges();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
        ...
        <div class="relative flex-1 max-w-md">
            <input
                ...
                placeholder="Search ACME challenges..."
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="challenge in paginatedChallenges" :key="challenge.metadata.name + challenge.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="challenge.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getNamespace(challenge)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getState(challenge)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getDomain(challenge)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(challenge.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredChallenges.length === 0">
                        <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No ACME challenges found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No ACME challenges found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredChallenges.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredChallenges.length)"></span>
                of
                <span class="font-medium" x-text="filteredChallenges.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredChallenges.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredChallenges.length / perPage))" :disabled="currentPage >= Math.ceil(filteredChallenges.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
        ...
        function challengesList() {
            return {
                ...
                filterChallenges() {
                    ...
                    setTimeout(() => {
                        ...
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(challenge => this.selectedNamespaces.includes(this.getNamespace(challenge)));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(challenge => {
                                const name = (challenge.metadata.name || '').toLowerCase();
                                const namespace = this.getNamespace(challenge).toLowerCase();
                                const state = this.getState(challenge).toLowerCase();
                                const domain = this.getDomain(challenge).toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower) ||
                                       state.includes(searchLower) || domain.includes(searchLower);
                            });
                        }
                ...
                updatePagination() {
                    ...
                    this.paginatedChallenges = this.filteredChallenges.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredChallenges.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredChallenges.length / this.perPage);
                    const maxVisible = 5;
                ...
                getNamespace(challenge) {
                    return (challenge.metadata && challenge.metadata.namespace) || 'default';
                },

                getState(challenge) {
                    return (challenge.status && challenge.status.state) || 'N/A';
                },

                getDomain(challenge) {
                    return (challenge.spec && challenge.spec.dnsName) || 'N/A';
                },
