<div x-data="issuersList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Issuers</h1>
        <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterIssuers();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterIssuers();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-8 text-center">
            <p class="text-gray-600">Please select a cluster from the dropdown to view issuers.</p>
        </div>
        @else
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="issuer in paginatedIssuers" :key="issuer.metadata.name + issuer.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="issuer.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getNamespace(issuer)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span :class="isReady(issuer) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'"
                                      class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                      x-text="isReady(issuer) ? 'Yes' : 'No'">
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(issuer.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredIssuers.length === 0">
                        <td colspan="4" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No issuers found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No issuers found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredIssuers.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredIssuers.length)"></span>
                of
                <span class="font-medium" x-text="filteredIssuers.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredIssuers.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredIssuers.length / perPage))" :disabled="currentPage >= Math.ceil(filteredIssuers.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
        ...
        function issuersList() {
            return {
                ...
                updatePagination() {
                    ...
                    this.paginatedIssuers = this.filteredIssuers.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredIssuers.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredIssuers.length / this.perPage);
                    const maxVisible = 5;
                ...
                getNamespace(issuer) {
                    return (issuer.metadata && issuer.metadata.namespace) || 'default';
                },

                isReady(issuer) {
                    if (!issuer.status || !issuer.status.conditions) return false;
