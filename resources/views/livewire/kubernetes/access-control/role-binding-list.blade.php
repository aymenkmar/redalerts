<div x-data="roleBindingsList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterRoleBindings();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterRoleBindings();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-8 text-center">
            <p class="text-gray-600">Please select a cluster from the dropdown to view role bindings.</p>
        </div>
        @else
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="roleBinding in paginatedRoleBindings" :key="roleBinding.metadata.name + roleBinding.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="roleBinding.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div x-show="hasRoleBindingWarnings(roleBinding)" class="flex justify-center" :title="getRoleBindingWarnings(roleBinding)">
                                    <svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getNamespace(roleBinding)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatBindings(roleBinding)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(roleBinding.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredRoleBindings.length === 0">
                        <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No role bindings found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No role bindings found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredRoleBindings.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredRoleBindings.length)"></span>
                of
                <span class="font-medium" x-text="filteredRoleBindings.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredRoleBindings.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredRoleBindings.length / perPage))" :disabled="currentPage >= Math.ceil(filteredRoleBindings.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function roleBindingsList() {
            return {
                allRoleBindings: @json($roleBindings),
                namespaces: @json($namespaces),
                filteredRoleBindings: [],
                paginatedRoleBindings: [],
                searchTerm: '',
                ...
                init() { this.filterRoleBindings(); },

                filterRoleBindings() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allRoleBindings];
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(roleBinding => this.selectedNamespaces.includes(this.getNamespace(roleBinding)));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(roleBinding => {
                                const name = (roleBinding.metadata.name || '').toLowerCase();
                                const namespace = this.getNamespace(roleBinding).toLowerCase();
                                const bindings = this.formatBindings(roleBinding).toLowerCase();
                                const warnings = this.getRoleBindingWarnings(roleBinding).toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower) ||
                                       bindings.includes(searchLower) || warnings.includes(searchLower);
                            });
                        }
                        this.filteredRoleBindings = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedRoleBindings = this.filteredRoleBindings.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredRoleBindings.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredRoleBindings.length / this.perPage);
                    const maxVisible = 5;
                ...
                getNamespace(roleBinding) {
                    return (roleBinding.metadata && roleBinding.metadata.namespace) || 'default';
                },

                formatBindings(roleBinding) {
                    if (!roleBinding.subjects || !Array.isArray(roleBinding.subjects)) {
                        return 'No subjects';
                    }
                    return roleBinding.subjects.map(subject => {
                ...
                getRoleBindingWarnings(roleBinding) {
                    const warnings = [];
                    ...
                    // Check for cluster-admin role bindings (very dangerous)
                    if (roleBinding.roleRef && roleBinding.roleRef.name === 'cluster-admin') {
                ...
                hasRoleBindingWarnings(roleBinding) {
                    const warnings = this.getRoleBindingWarnings(roleBinding);
                    return warnings !== '-';
                }
            }
        }
