<div x-data="namespacesList()" x-init="init()">
    ...
    <!-- Search Input (No namespace filter since this IS the namespaces page) -->
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-8 text-center">
            <p class="text-gray-600">Please select a cluster from the dropdown to view namespaces.</p>
        </div>
        @else
        ...
        <div x-show="!clientLoading && filteredNamespaces.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredNamespaces.length)"></span>
                of
                <span class="font-medium" x-text="filteredNamespaces.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredNamespaces.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredNamespaces.length / perPage))" :disabled="currentPage >= Math.ceil(filteredNamespaces.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
        ...
        function namespacesList() {
            return {
                ...
                updatePagination() {
                    ...
                    this.paginatedNamespaces = this.filteredNamespaces.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredNamespaces.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredNamespaces.length / this.perPage);
                    const maxVisible = 5;
                ...
                formatLabels(labels) {
                    ...
                    const labelArray = Object.entries(labels).map(([key, value]) => `${key}=${value}`);
                    return labelArray.length > 0 ? labelArray.join(', ') : '—';
                },
