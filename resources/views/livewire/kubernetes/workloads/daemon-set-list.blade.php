<div x-data="daemonSetsList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Daemon Sets</h1>
        <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterDaemonSets();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterDaemonSets();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Up-To-Date</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Available</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Node Selector</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="daemonSet in paginatedDaemonSets" :key="daemonSet.metadata.name + daemonSet.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(daemonSet.status && daemonSet.status.numberReady) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(daemonSet.status && daemonSet.status.updatedNumberScheduled) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(daemonSet.status && daemonSet.status.numberAvailable) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getNodeSelector(daemonSet)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(daemonSet.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>
        ...
        <div x-show="!clientLoading && filteredDaemonSets.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredDaemonSets.length)"></span>
                of
                <span class="font-medium" x-text="filteredDaemonSets.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredDaemonSets.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredDaemonSets.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredDaemonSets.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        function daemonSetsList() {
            return {
                    ...
                    setTimeout(() => {
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(daemonSet => {
                                ...
                                const namespace = (daemonSet.metadata.namespace || 'default').toLowerCase();
                                const nodeSelector = this.getNodeSelector(daemonSet).toLowerCase();

                                return name.includes(searchLower) ||
                                       namespace.includes(searchLower) ||
                                       nodeSelector.includes(searchLower);
                            });
                        }

                        this.filteredDaemonSets = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedDaemonSets = this.filteredDaemonSets.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredDaemonSets.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredDaemonSets.length / this.perPage);
                    const maxVisible = 5;
                ...
                getNodeSelector(daemonSet) {
                    const nodeSelector = daemonSet.spec?.template?.spec?.nodeSelector || {};
                    const selectorPairs = Object.entries(nodeSelector).map(([key, value]) => `${key}=${value}`);
