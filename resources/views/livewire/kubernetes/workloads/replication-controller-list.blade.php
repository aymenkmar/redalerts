<div x-data="replicationControllersList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Replication Controllers</h1>
        <button
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center mb-6">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterReplicationControllers();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterReplicationControllers();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
        ...
        <div class="relative flex-1 max-w-md">
            <input
                type="text"
                x-model="searchTerm"
                @input="filterReplicationControllers()"
                placeholder="Search replication controllers..."
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="replicationController in paginatedReplicationControllers" :key="replicationController.metadata.name + replicationController.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="replicationController.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="replicationController.metadata.namespace || 'default'"></td>
        ...
        <div x-show="!clientLoading && filteredReplicationControllers.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredReplicationControllers.length)"></span>
                of
                <span class="font-medium" x-text="filteredReplicationControllers.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredReplicationControllers.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredReplicationControllers.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredReplicationControllers.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <script>
        function replicationControllersList() {
            return {
                // Data from Livewire
                allReplicationControllers: @json($replicationControllers),
                namespaces: @json($namespaces),

                // Client-side state
                filteredReplicationControllers: [],
                paginatedReplicationControllers: [],
                searchTerm: '',
                ...
                filterReplicationControllers() {
                    ...
                    setTimeout(() => {
                        let filtered = [...this.allReplicationControllers];

                        // Filter by namespace
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(replicationController =>
                                this.selectedNamespaces.includes(replicationController.metadata.namespace || 'default')
                            );
                        }
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(replicationController => {
                                const name = (replicationController.metadata.name || '').toLowerCase();
                                const namespace = (replicationController.metadata.namespace || 'default').toLowerCase();
                                const selector = this.getSelector(replicationController).toLowerCase();

                                return name.includes(searchLower) ||
                                       namespace.includes(searchLower) ||
                                       selector.includes(searchLower);
                            });
                        }

                        this.filteredReplicationControllers = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedReplicationControllers = this.filteredReplicationControllers.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredReplicationControllers.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredReplicationControllers.length / this.perPage);
                    const maxVisible = 5;
                ...
                getSelector(replicationController) {
                    const selector = replicationController.spec?.selector || {};
                    const selectorPairs = Object.entries(selector).map(([key, value]) => `${key}=${value}`);
