<div x-data="cronJobsList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterCronJobs();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterCronJobs();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading && filteredCronJobs.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredCronJobs.length)"></span>
                of
                <span class="font-medium" x-text="filteredCronJobs.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredCronJobs.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredCronJobs.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredCronJobs.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        function cronJobsList() {
            return {
                    ...
                    setTimeout(() => {
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(cronJob => {
                                ...
                                const namespace = (cronJob.metadata.namespace || 'default').toLowerCase();
                                const schedule = ((cronJob.spec && cronJob.spec.schedule) || '').toLowerCase();
                                const timeZone = ((cronJob.spec && cronJob.spec.timeZone) || 'UTC').toLowerCase();

                                return name.includes(searchLower) ||
                                       namespace.includes(searchLower) ||
                                       schedule.includes(searchLower) ||
                                       timeZone.includes(searchLower);
                            });
                        }

                        this.filteredCronJobs = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedCronJobs = this.filteredCronJobs.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredCronJobs.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredCronJobs.length / this.perPage);
                    const maxVisible = 5;
                ...
                getActiveJobs(cronJob) {
                    if (cronJob.status && cronJob.status.active) {
                ...
                getLastScheduleTime(cronJob) {
                    if (cronJob.status && cronJob.status.lastScheduleTime) {
                        const date = new Date(cronJob.status.lastScheduleTime);
