            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $this->formatAge($deployment['metadata']['creationTimestamp']) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                $available = $deployment['status']['availableReplicas'] ?? 0;
                                $desired = $deployment['status']['replicas'] ?? 0;
                                $status = ($available == $desired && $desired > 0) ? 'Running' : 'Not Ready';
                                $statusColor = $status === 'Running' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800';
                            @endphp
