<div x-data="deploymentsList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterDeployments();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterDeployments();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="deployment in paginatedDeployments" :key="deployment.metadata.name + deployment.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="deployment.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div x-show="hasDeploymentWarnings(deployment)" class="flex justify-center" :title="getDeploymentWarnings(deployment)">
                                    <svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="deployment.metadata.namespace || 'default'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getPodsStatus(deployment)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(deployment.spec && deployment.spec.replicas) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(deployment.metadata.creationTimestamp)"></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                                    :class="getDeploymentStatusClass(deployment)"
                                    x-text="getDeploymentStatus(deployment)">
                                </span>
                            </td>
                        </tr>
                    </template>
                    ...
                    <!-- Empty state -->
                    <tr x-show="filteredDeployments.length === 0">
                        <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No deployments found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No deployments found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredDeployments.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredDeployments.length)"></span>
                of
                <span class="font-medium" x-text="filteredDeployments.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredDeployments.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredDeployments.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredDeployments.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        function deploymentsList() {
            return {
                    ...
                    setTimeout(() => {
                        ...
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(deployment =>
                                this.selectedNamespaces.includes(deployment.metadata.namespace || 'default')
                            );
                        }
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(deployment => {
                                ...
                                const namespace = (deployment.metadata.namespace || 'default').toLowerCase();
                                const status = this.getDeploymentStatus(deployment).toLowerCase();
                ...
                updatePagination() {
                    ...
                    this.paginatedDeployments = this.filteredDeployments.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredDeployments.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredDeployments.length / this.perPage);
                    const maxVisible = 5;
                ...
                getDeploymentStatus(deployment) {
                    ...
                    return (available === desired && desired > 0) ? 'Running' : 'Not Ready';
                },

                getDeploymentStatusClass(deployment) {
                    const status = this.getDeploymentStatus(deployment);
                    return status === 'Running' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800';
                },

                getDeploymentWarnings(deployment) {
                    const warnings = [];

                    // Check if deployment has insufficient replicas
                    const available = (deployment.status?.availableReplicas) || 0;
                    const desired = (deployment.status?.replicas) || 0;
                    ...
                    if (deployment.status?.conditions) {
                        deployment.status.conditions.forEach(condition => {
                            if (condition.status === 'False' && ['Available', 'Progressing'].includes(condition.type)) {
                    ...
                    if (deployment.spec?.paused) {
                        warnings.push('Deployment is paused');
                    }
