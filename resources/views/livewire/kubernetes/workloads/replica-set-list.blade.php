<div x-data="replicaSetsList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Replica Sets</h1>
        <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterReplicaSets();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterReplicaSets();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="replicaSet in paginatedReplicaSets" :key="replicaSet.metadata.name + replicaSet.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="replicaSet.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div x-show="hasReplicaSetWarnings(replicaSet)" class="flex justify-center" :title="getReplicaSetWarnings(replicaSet)">
                                    <svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="replicaSet.metadata.namespace || 'default'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(replicaSet.spec && replicaSet.spec.replicas) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(replicaSet.status && replicaSet.status.replicas) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(replicaSet.status && replicaSet.status.readyReplicas) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(replicaSet.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredReplicaSets.length === 0">
                        <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No replica sets found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No replica sets found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredReplicaSets.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredReplicaSets.length)"></span>
                of
                <span class="font-medium" x-text="filteredReplicaSets.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredReplicaSets.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredReplicaSets.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredReplicaSets.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <script>
        function replicaSetsList() {
            return {
                // Data from Livewire
                allReplicaSets: @json($replicaSets),
                namespaces: @json($namespaces),

                // Client-side state
                filteredReplicaSets: [],
                paginatedReplicaSets: [],
                searchTerm: '',
                ...
                filterReplicaSets() {
                    ...
                    setTimeout(() => {
                        let filtered = [...this.allReplicaSets];

                        // Filter by namespace
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(replicaSet =>
                                this.selectedNamespaces.includes(replicaSet.metadata.namespace || 'default')
                            );
                        }
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(replicaSet => {
                                ...
                                const namespace = (replicaSet.metadata.namespace || 'default').toLowerCase();
                        ...
                        this.filteredReplicaSets = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedReplicaSets = this.filteredReplicaSets.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredReplicaSets.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredReplicaSets.length / this.perPage);
                    const maxVisible = 5;
                ...
                getReplicaSetWarnings(replicaSet) {
                    ...
                    // Check if replica set has insufficient replicas
                    ...
                    if (current < desired) {
                        warnings.push(`${current}/${desired} replicas current`);
                    }

                    if (ready < desired) {
                        warnings.push(`${ready}/${desired} replicas ready`);
                    }

                    if (available < desired) {
                        warnings.push(`${available}/${desired} replicas available`);
                    }

                    // Check replica set conditions
                    if (replicaSet.status?.conditions) {
                        replicaSet.status.conditions.forEach(condition => {
                            if (condition.status === 'False' && ['ReplicaFailure'].includes(condition.type)) {
                ...
                hasReplicaSetWarnings(replicaSet) {
                    ...
                    if (current < desired || ready < desired || available < desired) {
                        return true;
                    }
