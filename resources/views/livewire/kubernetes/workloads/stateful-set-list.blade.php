<div x-data="statefulSetsList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Stateful Sets</h1>
        <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterStatefulSets();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterStatefulSets();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="statefulSet in paginatedStatefulSets" :key="statefulSet.metadata.name + statefulSet.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="statefulSet.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div x-show="hasStatefulSetWarnings(statefulSet)" class="flex justify-center" :title="getStatefulSetWarnings(statefulSet)">
                                    <svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="statefulSet.metadata.namespace || 'default'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getPodsStatus(statefulSet)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(statefulSet.spec && statefulSet.spec.replicas) || 0"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(statefulSet.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredStatefulSets.length === 0">
                        <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No stateful sets found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No stateful sets found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredStatefulSets.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredStatefulSets.length)"></span>
                of
                <span class="font-medium" x-text="filteredStatefulSets.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredStatefulSets.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredStatefulSets.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredStatefulSets.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <script>
        function statefulSetsList() {
            return {
                // Data from Livewire
                allStatefulSets: @json($statefulSets),
                namespaces: @json($namespaces),

                // Client-side state
                filteredStatefulSets: [],
                paginatedStatefulSets: [],
                searchTerm: '',
                    ...
                    setTimeout(() => {
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(statefulSet => {
                                ...
                                const namespace = (statefulSet.metadata.namespace || 'default').toLowerCase();
                        ...
                        this.filteredStatefulSets = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedStatefulSets = this.filteredStatefulSets.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredStatefulSets.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredStatefulSets.length / this.perPage);
                    const maxVisible = 5;
                ...
                getStatefulSetWarnings(statefulSet) {
                    ...
                    // Check if stateful set has insufficient replicas
                    const ready = (statefulSet.status?.readyReplicas) || 0;
                    ...
                    if (updated < desired) {
                        warnings.push(`${updated}/${desired} replicas updated`);
                    }

                    // Check stateful set conditions
                ...
                hasStatefulSetWarnings(statefulSet) {
                    ...
                    // Check stateful set conditions
                    if (statefulSet.status?.conditions) {
                        for (const condition of statefulSet.status.conditions) {
                            if (condition.status === 'False' && condition.type === 'Available') {
                                return true;
                            }
                        }
