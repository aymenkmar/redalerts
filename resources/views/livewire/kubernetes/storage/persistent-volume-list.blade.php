<div x-data="pvList()" x-init="init()">
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center mb-6">
        <!-- Namespace Filter (for filtering by claim namespace) -->
        <div class="relative">
            <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterPVs();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterPVs();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading && filteredPVs.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredPVs.length)"></span>
                of
                <span class="font-medium" x-text="filteredPVs.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredPVs.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredPVs.length / perPage))" :disabled="currentPage >= Math.ceil(filteredPVs.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
        ...
        function pvList() {
            return {
                ...
                filterPVs() {
                    ...
                    setTimeout(() => {
                        let filtered = [...this.allPVs];

                        // Filter by claim namespace
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(pv => {
                                const claimNamespace = this.getClaimNamespace(pv);
                                return claimNamespace === '—' || this.selectedNamespaces.includes(claimNamespace);
                            });
                        }
                ...
                updatePagination() {
                    ...
                    this.paginatedPVs = this.filteredPVs.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredPVs.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredPVs.length / this.perPage);
                    const maxVisible = 5;
                ...
                getStatus(pv) {
                    return (pv.status && pv.status.phase) || 'Unknown';
                },
                ...
                getClaimNamespace(pv) {
                    if (pv.spec && pv.spec.claimRef && pv.spec.claimRef.namespace) {
                ...
                getCapacity(pv) {
                    return (pv.spec && pv.spec.capacity && pv.spec.capacity.storage) || '—';
                },

                getAccessModes(pv) {
                    if (pv.spec && pv.spec.accessModes && Array.isArray(pv.spec.accessModes)) {
                        return pv.spec.accessModes.join(', ');
                    }
                    return '—';
                },

                getStorageClass(pv) {
                    return (pv.spec && pv.spec.storageClassName) || '—';
                },

                getReclaimPolicy(pv) {
                    return (pv.spec && pv.spec.persistentVolumeReclaimPolicy) || 'Delete';
                },
