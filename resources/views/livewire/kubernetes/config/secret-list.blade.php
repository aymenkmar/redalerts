<div>
    <h1 class="text-2xl font-bold mb-6">Secrets</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if(isset($secret['data']))
                                {{ implode(', ', array_keys($secret['data'])) }}
                            @else
                                —
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if(isset($secret['metadata']['labels']))
                                @php
                                    $labels = [];
                                    foreach ($secret['metadata']['labels'] as $key => $value) {
                                        $labels[] = "$key: $value";
                                    }
                                @endphp
                                {{ implode(', ', $labels) }}
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $this->formatAge($secret['metadata']['creationTimestamp']) }}
                        </td>
                    </tr>
                    @empty
                    <tr>
