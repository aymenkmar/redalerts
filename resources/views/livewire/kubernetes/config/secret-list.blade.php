<div x-data="secretsList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterSecrets();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterSecrets();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="secret in paginatedSecrets" :key="secret.metadata.name + secret.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="secret.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div x-show="hasSecretWarning(secret)" class="inline-flex">
                                    <svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="secret.metadata.namespace || 'default'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getSecretLabels(secret)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getSecretKeys(secret)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getSecretType(secret)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(secret.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>
                    ...
                    <tr x-show="filteredSecrets.length === 0">
                        <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No secrets found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No secrets found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredSecrets.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredSecrets.length)"></span>
                of
                <span class="font-medium" x-text="filteredSecrets.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredSecrets.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredSecrets.length / perPage))" :disabled="currentPage >= Math.ceil(filteredSecrets.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function secretsList() {
            return {
                ...
                init() { this.filterSecrets(); },

                filterSecrets() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allSecrets];
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(secret => this.selectedNamespaces.includes(secret.metadata.namespace || 'default'));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(secret => {
                                const name = (secret.metadata.name || '').toLowerCase();
                                const namespace = (secret.metadata.namespace || 'default').toLowerCase();
                                const keys = this.getSecretKeys(secret).toLowerCase();
                                const labels = this.getSecretLabels(secret).toLowerCase();
                                const type = this.getSecretType(secret).toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower) || keys.includes(searchLower) || labels.includes(searchLower) || type.includes(searchLower);
                            });
                        }
                ...
                updatePagination() {
                    ...
                    this.paginatedSecrets = this.filteredSecrets.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredSecrets.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredSecrets.length / this.perPage);
                    const maxVisible = 5;
                ...
                getSecretKeys(secret) {
                    if (secret.data && typeof secret.data === 'object') {
                        return Object.keys(secret.data).join(', ');
                    }
                    return '—';
                },

                getSecretLabels(secret) {
                    if (secret.metadata && secret.metadata.labels && typeof secret.metadata.labels === 'object') {
                        return Object.entries(secret.metadata.labels).map(([key, value]) => `${key}: ${value}`).join(', ');
                    }
                    return 'None';
                },
                ...
                hasSecretWarning(secret) {
                    ...
                    if (secret.type === 'kubernetes.io/tls' && secret.data && secret.data['tls.crt']) {
                        try {
                            // Basic check - in a real implementation you'd decode and parse the certificate
                    ...
                    if (secret.type === 'kubernetes.io/tls') {
                        return !secret.data || !secret.data['tls.crt'] || !secret.data['tls.key'];
                    }

                    if (secret.type === 'kubernetes.io/dockerconfigjson') {
                        return !secret.data || !secret.data['.dockerconfigjson'];
                    }
