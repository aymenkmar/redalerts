<div x-data="hpaList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Horizontal Pod Autoscalers</h1>
        <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterHPAs();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterHPAs();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Max Pods</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="hpa in paginatedHPAs" :key="hpa.metadata.name + hpa.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="(hpa.spec && hpa.spec.maxReplicas) || 'N/A'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getHPAReplicas(hpa)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(hpa.metadata.creationTimestamp)"></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                                    :class="getHPAStatusClass(hpa)"
                                    x-text="getHPAStatus(hpa)">
                                </span>
                            </td>
                        </tr>
                    </template>
        ...
        <div x-show="!clientLoading && filteredHPAs.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredHPAs.length)"></span>
                of
                <span class="font-medium" x-text="filteredHPAs.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredHPAs.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredHPAs.length / perPage))" :disabled="currentPage >= Math.ceil(filteredHPAs.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function hpaList() {
            return {
                allHPAs: @json($horizontalPodAutoscalers),
                ...
                init() { this.filterHPAs(); },

                filterHPAs() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allHPAs];
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(hpa => this.selectedNamespaces.includes(hpa.metadata.namespace || 'default'));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(hpa => {
                                const name = (hpa.metadata.name || '').toLowerCase();
                                const namespace = (hpa.metadata.namespace || 'default').toLowerCase();
                                const metrics = this.getHPAMetrics(hpa).toLowerCase();
                                const status = this.getHPAStatus(hpa).toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower) || metrics.includes(searchLower) || status.includes(searchLower);
                            });
                        }
                        this.filteredHPAs = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedHPAs = this.filteredHPAs.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredHPAs.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredHPAs.length / this.perPage);
                    const maxVisible = 5;
                ...
                getHPAMetrics(hpa) {
                    if (hpa.status && hpa.status.currentCPUUtilizationPercentage) {
                        return hpa.status.currentCPUUtilizationPercentage + '%';
                    } else if (hpa.spec && hpa.spec.metrics) {
                        const metricTypes = hpa.spec.metrics.map(metric => metric.type || 'Unknown');
                ...
                getHPAReplicas(hpa) {
                    if (hpa.status && hpa.status.currentReplicas !== undefined && hpa.status.desiredReplicas !== undefined) {
                ...
                getHPAStatusClass(hpa) {
                    const status = this.getHPAStatus(hpa);
                    switch (status) {
                        case 'Stable':
