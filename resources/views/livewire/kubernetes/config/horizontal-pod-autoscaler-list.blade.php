<div>
    <h1 class="text-2xl font-bold mb-6">Horizontal Pod Autoscalers</h1>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($horizontalPodAutoscalers as $hpa)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @php
                                $metrics = 'N/A';
                                if (isset($hpa['status']['currentCPUUtilizationPercentage'])) {
                                    $metrics = $hpa['status']['currentCPUUtilizationPercentage'] . '%';
                                } elseif (isset($hpa['spec']['metrics'])) {
                                    $metricTypes = [];
                                    foreach ($hpa['spec']['metrics'] as $metric) {
                                        $metricTypes[] = $metric['type'] ?? 'Unknown';
                                    }
                                    $metrics = implode(', ', $metricTypes);
                                }
                            @endphp
                            {{ $metrics }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $hpa['spec']['minReplicas'] ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $hpa['spec']['maxReplicas'] ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if(isset($hpa['status']['currentReplicas']) && isset($hpa['status']['desiredReplicas']))
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $this->formatAge($hpa['metadata']['creationTimestamp']) }}
                        </td>
