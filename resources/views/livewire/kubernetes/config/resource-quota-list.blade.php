<div x-data="resourceQuotasList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterResourceQuotas();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterResourceQuotas();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="quota in paginatedResourceQuotas" :key="quota.metadata.name + quota.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="quota.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div x-show="hasResourceQuotaWarning(quota)" class="inline-flex">
                                    <svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="quota.metadata.namespace || 'default'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(quota.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredResourceQuotas.length === 0">
                        <td colspan="4" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No resource quotas found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No resource quotas found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredResourceQuotas.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredResourceQuotas.length)"></span>
                of
                <span class="font-medium" x-text="filteredResourceQuotas.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredResourceQuotas.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredResourceQuotas.length / perPage))" :disabled="currentPage >= Math.ceil(filteredResourceQuotas.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function resourceQuotasList() {
            return {
                allResourceQuotas: @json($resourceQuotas),
                namespaces: @json($namespaces),
                filteredResourceQuotas: [],
                paginatedResourceQuotas: [],
                searchTerm: '',
                ...
                init() { this.filterResourceQuotas(); },

                filterResourceQuotas() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allResourceQuotas];
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(quota => this.selectedNamespaces.includes(quota.metadata.namespace || 'default'));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(quota => {
                                const name = (quota.metadata.name || '').toLowerCase();
                                const namespace = (quota.metadata.namespace || 'default').toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower);
                            });
                        }
                        this.filteredResourceQuotas = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedResourceQuotas = this.filteredResourceQuotas.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredResourceQuotas.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredResourceQuotas.length / this.perPage);
                    const maxVisible = 5;
                ...
                hasResourceQuotaWarning(quota) {
                    // Check for resource quota warnings
                    if (!quota.status) {
                        return true; // No status information
                    }
                    ...
                    if (quota.status.hard && quota.status.used) {
                        for (const resource in quota.status.hard) {
                            ...
                            if (hard && used) {
                                // Parse numeric values (handle units like Mi, Gi, etc.)
