<div x-data="limitRangesList()" x-init="init()">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterLimitRanges();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterLimitRanges();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading && filteredLimitRanges.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredLimitRanges.length)"></span>
                of
                <span class="font-medium" x-text="filteredLimitRanges.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= Math.ceil(filteredLimitRanges.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                <button @click="goToPage(Math.ceil(filteredLimitRanges.length / perPage))" :disabled="currentPage >= Math.ceil(filteredLimitRanges.length / perPage)" class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" title="Go to last page">Last</button>
            </div>
    ...
    <script>
        function limitRangesList() {
            return {
                allLimitRanges: @json($limitRanges),
                namespaces: @json($namespaces),
                filteredLimitRanges: [],
                paginatedLimitRanges: [],
                searchTerm: '',
                ...
                init() { this.filterLimitRanges(); },

                filterLimitRanges() {
                    this.clientLoading = true;
                    setTimeout(() => {
                        let filtered = [...this.allLimitRanges];
                        if (!this.selectedNamespaces.includes('all') && this.selectedNamespaces.length > 0) {
                            filtered = filtered.filter(limitRange => this.selectedNamespaces.includes(limitRange.metadata.namespace || 'default'));
                        }
                        if (this.searchTerm.trim()) {
                            const searchLower = this.searchTerm.toLowerCase();
                            filtered = filtered.filter(limitRange => {
                                const name = (limitRange.metadata.name || '').toLowerCase();
                                const namespace = (limitRange.metadata.namespace || 'default').toLowerCase();
                                return name.includes(searchLower) || namespace.includes(searchLower);
                            });
                        }
                        this.filteredLimitRanges = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedLimitRanges = this.filteredLimitRanges.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredLimitRanges.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredLimitRanges.length / this.perPage);
                    const maxVisible = 5;
