<div x-data="configMapsList()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Config Maps</h1>
        <button
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   value="all"
                                   @change="if($event.target.checked) { selectedNamespaces = ['all']; } filterConfigMaps();"
                        ...
                        <template x-for="namespace in namespaces" :key="namespace">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       x-model="selectedNamespaces"
                                       :value="namespace"
                                       @change="if($event.target.checked && selectedNamespaces.includes('all')) { selectedNamespaces = selectedNamespaces.filter(ns => ns !== 'all'); } filterConfigMaps();"
                                       class="rounded border-gray-300 text-red-600 focus:ring-red-500">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div x-show="!clientLoading" class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <template x-for="configMap in paginatedConfigMaps" :key="configMap.metadata.name + configMap.metadata.namespace">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="configMap.metadata.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="configMap.metadata.namespace || 'default'"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="getConfigMapKeys(configMap)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatAge(configMap.metadata.creationTimestamp)"></td>
                        </tr>
                    </template>

                    <!-- Empty state -->
                    <tr x-show="filteredConfigMaps.length === 0">
                        <td colspan="4" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                            <span x-show="searchTerm || !selectedNamespaces.includes('all')">No config maps found matching your filters</span>
                            <span x-show="!searchTerm && selectedNamespaces.includes('all')">No config maps found</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div x-show="!clientLoading && filteredConfigMaps.length > 0" class="flex flex-col sm:flex-row justify-between items-center mt-4 px-6 py-3 bg-white border-t border-gray-200">
            <div class="text-sm text-gray-700 mb-2 sm:mb-0">
                Showing
                <span class="font-medium" x-text="((currentPage - 1) * perPage) + 1"></span>
                to
                <span class="font-medium" x-text="Math.min(currentPage * perPage, filteredConfigMaps.length)"></span>
                of
                <span class="font-medium" x-text="filteredConfigMaps.length"></span>
                results
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button
                    @click="goToPage(currentPage + 1)"
                    :disabled="currentPage >= Math.ceil(filteredConfigMaps.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Next
                </button>
                ...
                <!-- Last page button -->
                <button
                    @click="goToPage(Math.ceil(filteredConfigMaps.length / perPage))"
                    :disabled="currentPage >= Math.ceil(filteredConfigMaps.length / perPage)"
                    class="px-3 py-1 rounded border bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <script>
        function configMapsList() {
            return {
                // Data from Livewire
                allConfigMaps: @json($configMaps),
                    ...
                    setTimeout(() => {
                        ...
                        if (this.searchTerm.trim()) {
                            ...
                            filtered = filtered.filter(configMap => {
                                ...
                                const namespace = (configMap.metadata.namespace || 'default').toLowerCase();
                                const keys = this.getConfigMapKeys(configMap).toLowerCase();
                        ...
                        this.filteredConfigMaps = filtered;
                        this.currentPage = 1;
                ...
                updatePagination() {
                    ...
                    this.paginatedConfigMaps = this.filteredConfigMaps.slice(start, end);
                },

                goToPage(page) {
                    const maxPage = Math.ceil(this.filteredConfigMaps.length / this.perPage);
                ...
                getVisiblePages() {
                    const totalPages = Math.ceil(this.filteredConfigMaps.length / this.perPage);
                    const maxVisible = 5;
                ...
                getConfigMapKeys(configMap) {
                    if (configMap.data && typeof configMap.data === 'object') {
                        return Object.keys(configMap.data).join(', ');
                    }
                    return '—';
                },
