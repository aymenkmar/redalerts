<div>
    <h1 class="text-2xl font-bold mb-6">Config Maps</h1>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($configMaps as $configMap)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if(isset($configMap['data']))
                                {{ implode(', ', array_keys($configMap['data'])) }}
                            @else
                                —
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $this->formatAge($configMap['metadata']['creationTimestamp']) }}
                        </td>
                    </tr>
                    @empty
                    <tr>
