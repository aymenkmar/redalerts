...
class JobList extends Component
{
    ...
    public function loadData()
    {
        ...
        try {
            ...
            if (isset($jobsResponse['items'])) {
                $this->jobs = $jobsResponse['items'];
            } else {
                $this->jobs = [];
                // If no items, check if there's an error in the response
                if (isset($jobsResponse['message'])) {
                    throw new \Exception('Kubernetes API error: ' . $jobsResponse['message']);
                }
            }
            ...
            $this->error = 'Failed to load jobs: ' . $e->getMessage();
        } finally {
            $this->loading = false;
        }
    }
    ...
    public function refreshData()
    {
        try {
            ...
            $this->dispatch('notify', [
                'type' => 'success',
                'message' => 'Jobs data refreshed successfully'
            ]);
        } catch (\Exception $e) {
            $this->dispatch('notify', [
                'type' => 'error',
    ...
    {
        return view('livewire.kubernetes.workloads.job-list')->layout('layouts.kubernetes');
    }
}
