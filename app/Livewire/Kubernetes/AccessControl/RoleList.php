...
class RoleList extends Component
{
    ...
    public function refreshData()
    {
        try {
            ...
            $this->dispatch('notify', [
                'type' => 'success',
                'message' => 'Roles data refreshed successfully'
            ]);
        } catch (\Exception $e) {
            $this->dispatch('notify', [
                'type' => 'error',
    ...
    public function getFilteredRolesProperty()
    {
        ...
        if (!in_array('all', $this->selectedNamespaces)) {
            $roles = $roles->filter(function ($role) {
                return in_array($role['metadata']['namespace'] ?? 'default', $this->selectedNamespaces);
            });
        }
        ...
        // Apply pagination
        $paginatedRoles = $roles->forPage($this->currentPage, $this->perPage);

        return $paginatedRoles->values()->all();
    }
    ...
    public function render()
    {
        try {
            ...
            return view('livewire.kubernetes.access-control.role-list', [
                'filteredRoles' => [],
                'error' => 'An error occurred while loading roles. Please try again.'
            ])->layout('layouts.kubernetes');
        }
    }
}
