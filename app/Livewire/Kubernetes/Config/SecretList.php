...
class SecretList extends Component
{
    ...
    public function loadData()
    {
        ...
        try {
            ...
            try {
                ...
                $secretsResponse = $service->getSecrets();
            } catch (\Exception $e) {
            ...
            if (isset($secretsResponse['items'])) {
                $this->secrets = $secretsResponse['items'];
            } else {
                $this->secrets = [];
                // If no items, check if there's an error in the response
                if (isset($secretsResponse['message'])) {
                    throw new \Exception('Kubernetes API error: ' . $secretsResponse['message']);
                }
            }
            ...
            $this->error = 'Failed to load secrets: ' . $e->getMessage();
        } finally {
            $this->loading = false;
        }
    }
    ...
    public function refreshData()
    {
        try {
            ...
            $this->dispatch('notify', [
                'type' => 'success',
                'message' => 'Secrets data refreshed successfully'
            ]);
        } catch (\Exception $e) {
            $this->dispatch('notify', [
                'type' => 'error',
    ...
    {
        return view('livewire.kubernetes.config.secret-list')->layout('layouts.kubernetes');
    }
}
