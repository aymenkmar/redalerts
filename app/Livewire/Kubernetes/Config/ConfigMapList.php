...
class ConfigMapList extends Component
{
    ...
    public function loadData()
    {
        ...
        try {
            ...
            try {
                ...
                $configMapsResponse = $service->getConfigMaps();
            } catch (\Exception $e) {
            ...
            if (isset($configMapsResponse['items'])) {
                $this->configMaps = $configMapsResponse['items'];
            } else {
                $this->configMaps = [];
                // If no items, check if there's an error in the response
                if (isset($configMapsResponse['message'])) {
                    throw new \Exception('Kubernetes API error: ' . $configMapsResponse['message']);
                }
            }
            ...
            $this->error = 'Failed to load config maps: ' . $e->getMessage();
        } finally {
            $this->loading = false;
        }
    }
    ...
    public function refreshData()
    {
        try {
            ...
            $this->dispatch('notify', [
                'type' => 'success',
                'message' => 'Config maps data refreshed successfully'
            ]);
        } catch (\Exception $e) {
            $this->dispatch('notify', [
                'type' => 'error',
    ...
    {
        return view('livewire.kubernetes.config.config-map-list')->layout('layouts.kubernetes');
    }
}
