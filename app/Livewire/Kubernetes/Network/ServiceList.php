...
class ServiceList extends Component
{
    ...
    public function loadData()
    {
        ...
        try {
            ...
            try {
                ...
                $servicesResponse = $service->getServices();
            } catch (\Exception $e) {
            ...
            if (isset($servicesResponse['items'])) {
                $this->services = $servicesResponse['items'];
            } else {
                $this->services = [];
                // If no items, check if there's an error in the response
                if (isset($servicesResponse['message'])) {
                    throw new \Exception('Kubernetes API error: ' . $servicesResponse['message']);
                }
            }
            ...
            $this->error = 'Failed to load services: ' . $e->getMessage();
        } finally {
            $this->loading = false;
        }
    }
    ...
    {
        return view('livewire.kubernetes.network.service-list')->layout('layouts.kubernetes');
    }
}
