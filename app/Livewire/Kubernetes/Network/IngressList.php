...
class IngressList extends Component
{
    ...
    public function loadData()
    {
        ...
        try {
            ...
            try {
                ...
                $ingressesResponse = $service->getIngresses();
            } catch (\Exception $e) {
                $service = new \App\Services\KubernetesService($kubeconfigPath);
                $ingressesResponse = $service->getIngresses();
            }

            if (isset($ingressesResponse['items'])) {
                $this->ingresses = $ingressesResponse['items'];
            } else {
                $this->ingresses = [];
                if (isset($ingressesResponse['message'])) {
                    throw new \Exception('Kubernetes API error: ' . $ingressesResponse['message']);
                }
            }
            ...
            $this->error = 'Failed to load ingresses: ' . $e->getMessage();
        } finally {
            $this->loading = false;
        }
    }
    ...
    public function refreshData()
    {
        try {
            ...
            $this->dispatch('notify', [
                'type' => 'success',
                'message' => 'Ingresses data refreshed successfully'
            ]);
        } catch (\Exception $e) {
            $this->dispatch('notify', [
                'type' => 'error',
    ...
    {
        return view('livewire.kubernetes.network.ingress-list')->layout('layouts.kubernetes');
    }
}
